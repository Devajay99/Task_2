...
class Edrone():
	...
	def __init__(self):
		...
		self.Kp = [917*0.05,175*0.05,349*0.05]
		self.Ki = [9*0.009,0,0]
		self.Kd = [611*0.4,262*0.4,524*0.4]
	...
	#----------------------------- PID algorithm --------------------------------------------------------------
		...
		if(self.time_change>0.050):
			...
			#YOUR CODE HERE
			self.error_A = self.drone_position[0] - self.setpoint[0]
			self.error_B = self.drone_position[1] - self.setpoint[1]
			self.error_C = self.drone_position[2] - self.setpoint[2]
			#Publish the errors
			
			#YOUR CODE HERE
			self.roll_error_pub.publish(self.error_A)
			self.pitch_error_pub.publish(self.error_B)
			self.alt_error_pub.publish(self.error_C)
			#Step 2- Compute error, change_in_error
			#YOUR CODE HERE
			self.error = [self.error_A,self.error_B,self.error_C]
			...
			self.cmd.rcRoll = 1500 - self.out_roll #YOUR CODE HERE
			self.cmd.rcThrottle = 1500 + self.out_alt #YOUR CODE HERE
